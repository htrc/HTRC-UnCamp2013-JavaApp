...
public class UnCampPageDemo {	
	
	// service end points 
	private static final String DATA_API_ENDPOINT = "https://sandbox.htrc.illinois.edu:25443/data-api";
	private static final String SOLR_PROXY_URL = "http://sandbox.htrc.illinois.edu:9446/solr/meta/select?q=";
	...
	private static final String PAGE_URL = "/pages";	
	...
	private static InputStream getDataFromDataAPI(String dataapiEPR, String requestUrl, 
		List<String> volumeIDs, String token, boolean concat) throws Exception {
		URL url = null;
        ...
        // hard code page id to request body
        String pageid = "[1,5,10]";
        StringBuilder bodyBuilder = new StringBuilder();
        bodyBuilder.append("pageIDs=").append(URLEncoder.encode(volumeIDs.get(0) + pageid, "UTF-8"));
        for (int i = 1; i < volumeIDs.size(); i++) {
            bodyBuilder.append(URLEncoder.encode("|", "UTF-8")).
            	append(URLEncoder.encode(volumeIDs.get(i) + pageid, "UTF-8"));
        }
        
        // optional parameter to allow concatenation of the pages for each volume
        if (concat) {
	 ...
	 * OAuth2 server, obtains an OAuth2 token, requests for volume contents from 
	 * Data API, and saves the volume contents to a ZIP file.
	 */
	public static void main(String[] args) throws Exception {
		if (args.length < 4) {
			System.out.println("UnCampPageDemo <client id> <client secret> <solr query string> <zip file name> [concat]");
			System.exit(-1);
		}
		...
		if (args.length == 5) concat = Boolean.valueOf(args[4]); 
		...
		try {
			...
			InputStream responseStream = getDataFromDataAPI(DATA_API_ENDPOINT, PAGE_URL, volumeList, token, concat);
